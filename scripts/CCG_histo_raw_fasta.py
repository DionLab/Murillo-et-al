#!/usr/bin/env python3
"""
Plot CCG count histograms for each FASTA file in the input directory.
Works on both raw demultiplexed HiFi FASTAs and extracted CCG7/CCG10 FASTAs.
...
FASTA_DIR = "/scratch/SCWF00044/Ruban/Lymphoblastoids_proline_phasing/260510/PB045"
...
def main():
    ...
    for fasta_path in fastas:
        ...
        title = f"CCG distribution — {bc} ({len(counts)}/{total} reads with HTT anchors)"
        out_path = os.path.join(PLOT_DIR, f"{bc}_ccg_histogram.png")
        plot_histogram(counts, title, out_path)
        print(f"{bc}: {len(counts)} HTT reads / {total} total -> {out_path}")
